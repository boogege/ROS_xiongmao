...
1.976,-2.453,-0.999,0.033,
1.282,-3.837,-0.039,0.999,
2.632,-5.412,-0.616,0.788,
1.302,-4.230,0.745,0.667,
-1.848,-2.360,-0.885,0.465,
2.632,-5.412,-0.616,0.788,
1.302,-4.230,0.745,0.667
};
...
void send_vel()
{
	...
	else if(current_point==3||current_point==6||current_point==7)  //碰水泥柱 右边
	{	
	...
	{	
		...
		if ((current_status==3)&&(last_current_status!=current_status))
		{
			...
			switch(current_point)
			{
				...
				case 1:vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();mcu_cmd=CATCH_1;sendMcu();sleep(1);vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
				case 2:vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();sleep(1);vel_select=LEFT_BACK;back=true;send_vel();back=false;mcu_cmd=DOWN;sendMcu();break;
				case 3: vel_select=RIGHT_GO;send_vel();mcu_cmd=CATCH_2;sendMcu();mcu_cmd=UP;sendMcu();sleep(3);vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 4: mcu_cmd=PUT_2;sendMcu();mcu_cmd=PUT_1;sendMcu();mcu_cmd=DOWN;sendMcu();vel_select=LEFT_GO;send_vel();mcu_cmd=CATCH_1;sendMcu();sleep(2);mcu_cmd=UP;sendMcu();sleep(3);vel_select=LEFT_BACK;back=true;send_vel();back=false;mcu_cmd=PUT_1;sendMcu();mcu_cmd=DOWN;sendMcu();break;
				case 5:vel_select=RIGHT_GO;send_vel();mcu_cmd=CATCH_2;sendMcu();sleep(1);mcu_cmd=UP;sendMcu();vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 6:vel_select=RIGHT_GO;send_vel();mcu_cmd=PUT_2;sendMcu();sleep(1);mcu_cmd=CATCH_2;sendMcu();vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 7:vel_select=RIGHT_GO;send_vel();mcu_cmd=PUT_2;sendMcu();sleep(1);vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
