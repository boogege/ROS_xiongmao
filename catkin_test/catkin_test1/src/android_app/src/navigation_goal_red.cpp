...
double nav_point[10][4]={2.137,0.019,-0.47,0.883,
4.723,-1.371,-0.994,0.112,
2.949,-2.926,-0.487,0.873,
3.312,-3.623,-0.001,0.999,
0.158,-4.56,-0.907,0.42,
1.502,-5.61,-0.658,0.753,
-1.667,-1.803,0.715,0.699,
-0.681,-2.159,-0.36,0.933};
...
void pub_switch()
{
	if(current_point>=0&&current_point<=7)	
	{
...
void send_vel()
{
	...
	dura = end-begin;
	printf("延时%ld\n",dura);
    	if(current_point==0||current_point==6) //左边抓地标
	{	if(!back)
		{
			msg.linear.x = -0.1;
			msg.linear.y = 0.1732;
	...
	else if(current_point==2||current_point==3)
	{	
		if(!back)  //向前
		{	
			if(crash_3&&crash_4)
	...
	else if(current_point==4||current_point==5||current_point==7)
	{	
		if(!back)  //向前
		{	
			if(crash_1&&crash_2)
...
void statusCallback(const actionlib_msgs::GoalStatusArray::ConstPtr& msg)
{
	...
	{	
		...
		if ((current_status==3)&&(last_current_status!=current_status))
		{
			...
			{
				case 0: vel_select=LEFT_GO;send_vel();mcu_cmd=CATCH_1;sendMcu();sleep(1);mcu_cmd=UP;sendMcu();vel_select=LEFT_BACK;back=true;send_vel();mcu_cmd=DOWN;sendMcu();back=false;break;
				case 1: vel_select=RIGHT_GO;send_vel();mcu_cmd=CATCH_2;sendMcu();sleep(1);mcu_cmd=UP;sendMcu();vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 2: vel_select=RIGHT_GO;send_vel();mcu_cmd=PUT_2;sendMcu();mcu_cmd=CATCH_2;sendMcu();sleep(1);vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 3:vel_select=RIGHT_GO;send_vel();mcu_cmd=PUT_2;sendMcu();sleep(1);vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 4: case 7: vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();mcu_cmd=CATCH_1;sendMcu();sleep(1);vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
				case 5:vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();sleep(1);vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
				case 6: mcu_cmd=PUT_1;sendMcu();mcu_cmd=DOWN;sendMcu();sleep(3);vel_select=LEFT_GO;send_vel();mcu_cmd=CATCH_1;sendMcu();sleep(1);mcu_cmd=UP;sendMcu();vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
