...
void send_vel()
{
    ...
    begin=tv.tv_sec*1000+tv.tv_usec/1000;  //毫秒
    ...
    {   
	gettimeofday(&tv,NULL);
    	end=tv.tv_sec*1000+tv.tv_usec/1000;  //毫秒	
	dura = end - begin;
	printf("延时%ld\n",dura);
    	if(current_point==0)
	{	if(!back)
	...
	{	
		...
		if ((current_status==3)&&(last_current_status!=current_status))
		{
			...
			switch(current_point)
			{
				...
				case 2: vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();sleep(3);mcu_cmd=CATCH_1;sendMcu();vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
				case 3: vel_select=RIGHT_GO;send_vel();mcu_cmd=PUT_2;sendMcu();sleep(3);mcu_cmd=CATCH_2;sendMcu();vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 4: vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();sleep(3);mcu_cmd=CATCH_1;sendMcu();vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
				case 5: vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();sleep(3);mcu_cmd=CATCH_1;sendMcu();vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
