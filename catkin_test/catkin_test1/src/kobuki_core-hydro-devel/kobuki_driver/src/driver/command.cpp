...
Command Command::SetVelocityControl(const int16_t &vx, const int16_t &vy, const int16_t &wz)
{
  Command outgoing;
  outgoing.data.cmd_x = vx;
  outgoing.data.cmd_y = vy;
  outgoing.data.cmd_w = wz;
  outgoing.data.command = Command::BaseControl;
  return outgoing;
}
...
bool Command::serialise(ecl::PushAndPop<unsigned char> & byteStream)
{
  ...
  switch (data.command)
  {
    case BaseControl:
      ...
      buildBytes(data.cmd_x, byteStream);
      buildBytes(data.cmd_y, byteStream);
      buildBytes(data.cmd_w, byteStream);
      break;
    case Sound:
      buildBytes(cmd, byteStream);
