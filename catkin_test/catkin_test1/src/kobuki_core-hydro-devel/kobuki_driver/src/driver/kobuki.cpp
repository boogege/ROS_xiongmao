...
//sig_error.emit("kobuki.cpp: serial.open(parameters.device_port, ecl::BaudRate_115200, ecl::DataBits_8, ecl::StopBits_1, ecl::NoParity);");
  }
...
void Kobuki::spin()
{
 ...
 //sig_error.emit("----------in spin()---------");
  ...
  {
 //sig_error.emit("aaaaaaaaaaaaaaaa");
    /*********************
     ** Checking Connection
     **********************/
    if ( !serial.open() ) {
      try {
        sig_error.emit("-----------serial close---------------");
        ...
        */
          case Header::Inertia:
            ...
            //if (isnan(heading_offset) == true)
               //heading_offset = static_cast<double>(int16_t(inertia.data.angle));
            break;
        /*  case Header::Cliff:
...
ecl::Angle<double> Kobuki::getHeading() const
{
  ...
  heading = static_cast<double>(int16_t(inertia.data.angle) / 1000.0);
  return ecl::wrap_angle(heading);
}

double Kobuki::getAngularVelocity() const
{
  return (static_cast<double>(int16_t(inertia.data.angle_rate) / 1000.0));
}
...
void Kobuki::resetOdometry()
{
  ...
  //heading_offset = (static_cast<double>(int16_t(inertia.data.angle) / 1000.0));
}


void Kobuki::getWheelJointStates(double &wheel_left_s, double &wheel_left_cmd, 
			         double &wheel_right_s, double &wheel_right_cmd,
                                 double &wheel_back_s, double &wheel_back_cmd)
{
  diff_drive.getWheelJointStates(wheel_left_s, wheel_left_cmd, wheel_right_s, wheel_right_cmd, wheel_back_s, wheel_back_cmd);
}
...
{
  diff_drive.update(core_sensors.data.time_stamp, core_sensors.data.left_wheel, core_sensors.data.right_wheel, core_sensors.data.back_wheel,
...
void Kobuki::setBaseControl(const double &linear_x, const double &linear_y, const double &angular_w)
{
  diff_drive.setVelocityCommands(linear_x, linear_y, angular_w);
}
...
//  sig_error.emit("--------------down-------------------");
  ...
  //std::cout << "Vx: " << velocity_commands[0] << ", Vy: " << velocity_commands[1] << ", Wz: " << velocity_commands[2] <<std::endl;
...
//sig_error.emit("-----------------------------------------");
  ...
  //check_device();
  //sig_error.emit("rrrrrrrrrrrrrrrrrrrrrr");
 // serial.write((const char*)&command_buffer[0], command_buffer.size());
  //sig_error.emit("tttttttttttttttttttttttt");
