...
0.194,-4.88,0.959,0.284,   //右放

1.8297,-5.763,-0.436,0.8999 //左抓
};
...
void send_vel()
{
	...
	else if(current_point==1||current_point==2||current_point==5)  //碰水泥柱 左边
	{	
		if(!back)  //向前
		{	
			if(crash_1&&crash_2)
	...
	else if(current_point==4)  //碰水泥柱 右边
	{	
	...
	{	
		...
		if ((current_status==3)&&(last_current_status!=current_status))
		{
			...
			switch(current_point)
			{
				...
				case 3: vel_select=RIGHT_GO;send_vel();mcu_cmd=CATCH_2;sendMcu();sleep(1);mcu_cmd=UP;sendMcu();vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 4:vel_select=RIGHT_GO;send_vel();mcu_cmd=PUT_2;sendMcu();sleep(1);vel_select=RIGHT_BACK;back=true;send_vel();back=false;break;
				case 5: mcu_cmd=DOWN;sendMcu();vel_select=LEFT_GO;send_vel();mcu_cmd=CATCH_1;sendMcu();sleep(2);mcu_cmd=UP;sendMcu();sleep(3);vel_select=LEFT_BACK;back=true;send_vel();back=false;mcu_cmd=PUT_1;sendMcu();break;
