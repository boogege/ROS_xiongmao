...
-1.51,-1.882,0.551,0.835, //左抓
1.8297,-5.763,-0.436,0.8999,//左放
0.194,-4.88,0.959,0.284   //右抓
};
...
void send_vel()
{
	...
	dura = end-begin;
	printf("延时%ld\n",dura);
    	if(current_point==0||current_point==3)  //左边抓地标 
	{	if(!back)
		{
			msg.linear.x = -0.1;
			msg.linear.y = 0.1732;
	...
	else if(current_point==5)  //碰水泥柱 右边
	{	
	...
	{	
		...
		if ((current_status==3)&&(last_current_status!=current_status))
		{
			...
			switch(current_point)
			{
				...
				case 3: vel_select=LEFT_GO;send_vel();mcu_cmd=CATCH_1;sendMcu();sleep(1);mcu_cmd=UP;sendMcu();vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
				case 4:vel_select=LEFT_GO;send_vel();mcu_cmd=PUT_1;sendMcu();sleep(1);vel_select=LEFT_BACK;back=true;send_vel();back=false;break;
				case 5:mcu_cmd=DOWN;sendMcu();vel_select=RIGHT_GO;send_vel();mcu_cmd=CATCH_2;sendMcu();sleep(1);mcu_cmd=UP;sendMcu();sleep(3);vel_select=RIGHT_BACK;back=true;send_vel();back=false;mcu_cmd=PUT_2;sendMcu();break;
